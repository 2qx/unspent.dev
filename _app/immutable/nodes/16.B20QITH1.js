...
// }
//`}},Wt=["BCH_2026_05","BCH_SPEC"],Yt={$schema:Lt,description:Ft,name:qt,entities:Vt,scenarios:Xt,scripts:Mt,supported:Wt},Ht="@unspent/subscription",Kt={name:Ht};class B{static USER_AGENT=Kt.name;static PROTOCOL_IDENTIFIER="U3S";static EXECUTOR_FEE=5e3;static tokenAware=!0;static template=Yt;static compiler=kt(this.template);static vm=At();static dataToBytecode(e){return{installment:Y(e.installment),recipient:e.recipient,period:Y(BigInt(e.period)),auth:new Uint8Array(e.auth).reverse()}}static bytecodeToData(e){const t=yt(e.installment);if(typeof t!="bigint")throw t;return{installment:t,recipient:e.recipient,period:_t(e.period),auth:new Uint8Array(e.auth).reverse()}}static parseCommitment(e){typeof e=="string"&&(e=P(e));const t=Ut(e);if(Tt(t[0])!==this.PROTOCOL_IDENTIFIER)throw Error(`"Non-${typeof this} record NFT passed as ${typeof this}"`);return{installment:t[1],recipient:t[2],period:t[3],auth:t[4]}}static encodeCommitment(e){const t=this.dataToBytecode(e);return C(this.getLockingBytecode(t))}static getLockingBytecode(e){const t=this.compiler.generateBytecode({data:{bytecode:e},scriptId:"lock"});if(!t.success)throw new Error("Failed to generate bytecode, script: , "+JSON.stringify(t,null,"  "));return t.bytecode}static getScriptHash(e,t=!0){let n=this.dataToBytecode(e);return j(this.getLockingBytecode(n),t)}static getAddress(){throw Error("No Cashaddr for p2s contract type")}static getSourceOutput(e,t){let n=this.dataToBytecode(e);return{lockingBytecode:this.getLockingBytecode(n),valueSatoshis:BigInt(t.value),token:t.token_data?{category:P(t.token_data.category),amount:BigInt(t.token_data.amount)}:void 0}}static getInput(e,t,n){let c=this.dataToBytecode(e);return{outpointIndex:t.tx_pos,outpointTransactionHash:P(t.tx_hash),sequenceNumber:n,unlockingBytecode:{data:{bytecode:c},compiler:this.compiler,script:"execute",valueSatoshis:BigInt(t.value),token:t.token_data?{category:P(t.token_data.category),amount:BigInt(t.token_data.amount)}:void 0}}}static subscriptionToOutput(e){let t=this.dataToBytecode(e.data),n=this.getLockingBytecode(t),c=typeof e.category=="string"?P(e.category):e.category,u=BigInt(e.installmentCount*this.EXECUTOR_FEE),d=BigInt(e.installmentCount)*e.data.installment;return{lockingBytecode:n,valueSatoshis:u,token:{category:c,amount:d}}}static getInstallmentOutput(e,t){let n=e.installment;return BigInt(t.token_data?.amount)<n&&(n=BigInt(t.token_data?.amount)),{lockingBytecode:e.recipient,valueSatoshis:1000n,token:t.token_data?{category:P(t.token_data.category),amount:n}:void 0}}static getReturnOutput(e,t){return{lockingBytecode:{data:{bytecode:this.dataToBytecode(e)},compiler:this.compiler,script:"lock"},valueSatoshis:BigInt(t.value-this.EXECUTOR_FEE),token:t.token_data?{category:P(t.token_data.category),amount:BigInt(t.token_data.amount)-e.installment}:void 0}}static getExecutorOutput(e,t){let n=J(e);if(typeof n=="string")throw n;return{lockingBytecode:n.bytecode,valueSatoshis:BigInt(t)}}static getWalletInputs(e,t,n,c,u=[]){let d=[],h=[];if(c&&typeof c!="string"&&(c=C(c)),c?e=e.filter(s=>s.token_data&&s.token_data?.category==c).filter(s=>s.token_data&&!s.token_data.nft):e=e.filter(s=>!s.token_data),e.length==0)throw Error("no wallet utxos left, maximum recursion depth reached.");const l=Math.floor(Math.random()*e.length),f=e[l];e.splice(l,1),d.push(bt(f,n)),u.push(Pt(f,n));let r=N(u),m=U(u,c);if(c&&m<t||!c&&r<t){let s=this.getWalletInputs([...e],t,n,c,[...u]);d.push(...s.inputs),h.push(...s.outputs),u=s.sourceOutputs}return{inputs:d,outputs:h,sourceOutputs:u}}static getWalletLayers(e,t,n,c,u){let d=H(t.outputs),h=N(n),l=d-h;const f=this.getWalletInputs(c,l,u);t.inputs.push(...f.inputs),n.push(...f.sourceOutputs);let r=K(t.outputs,e),m=U(n,e),s=r-m;if(s>0n){const O=this.getWalletInputs(c,s,u,e);t.inputs.push(...O.inputs),n.push(...O.sourceOutputs)}d=H(t.outputs),h=N(n);let a=h-d;r=K(t.outputs,e),m=U(n,e);let i=m-r;return i>0&&(t.outputs.push(Q(1000n,i,e,u)),a-=1000n),t.outputs.push(Q(a,0n,void 0,u)),t}static setThreadLayers(e,t=[]){let n=[],c=[];if(new Set(e.map(d=>d.data)).size>1)throw"Cannot mix covenant types";return c.push(...e.map(d=>this.subscriptionToOutput(d))),{inputs:n,outputs:c,sourceOutputs:t}}static execute(e,t,n,c=1){const u=[],d=[];let h=[],l={locktime:0,version:2,inputs:u,outputs:d};for(let a of e){let i=this.parseCommitment(a.record);if(a.utxo.height>0&&a.utxo.value>5800){const O=t-a.utxo.height,p=this.bytecodeToData(i);O>=p.period&&BigInt(a.utxo.token_data?.amount)>=p.installment&&(l.inputs.push(this.getInput(p,a.utxo,O)),l.outputs.push(this.getInstallmentOutput(p,a.utxo)),h.push(this.getSourceOutput(p,a.utxo)))}}for(let a of e){let i=this.parseCommitment(a.record);if(a.utxo.height>0&&a.utxo.value>5800){const O=t-a.utxo.height,p=this.bytecodeToData(i);O>=p.period&&BigInt(a.utxo.token_data?.amount)>=p.installment&&l.outputs.push(this.getReturnOutput(p,a.utxo))}}for(let a of e){let i=this.parseCommitment(a.record);if(a.utxo.height>0&&a.utxo.value>5800){const O=t-a.utxo.height,p=this.bytecodeToData(i);O>=p.period&&BigInt(a.utxo.token_data?.amount)<p.installment&&(l.inputs.push(this.getInput(p,a.utxo,O)),l.outputs.push(this.getInstallmentOutput(p,a.utxo)),h.push(this.getSourceOutput(p,a.utxo)))}}n&&l.inputs.length>0&&l.outputs.push(this.getExecutorOutput(n,l.inputs.length*(this.EXECUTOR_FEE-1e3)));let f=x(l);if(!f.success)throw new Error("generate transaction failed!, errors: "+JSON.stringify(f.errors,null,"  "));if(n&&l.inputs.length>0){const a=$(f.transaction,c)+1n,i=l.outputs.length-1;l.outputs[i].valueSatoshis=l.outputs[i].valueSatoshis-a}if(f=x(l),!f.success)throw new Error("generate transaction failed!, errors: "+JSON.stringify(f.errors,null,"  "));const r=f.transaction,m=z(r,h,{maximumTokenCommitmentLength:40});if(m!==!0)throw m;let s=this.vm.verify({sourceOutputs:h,transaction:r});if(typeof s=="string")throw s;return{sourceOutputs:h,transaction:r,verify:s}}static administer(e,t,n,c,u,d=0,h=1){let r={locktime:0,version:2,inputs:[],outputs:[]};e.token_data.category;const m=It(e,u,d);r.inputs.push(...m.inputs),r.outputs.push(...m.outputs);let s=m.sourceOutputs;const a=this.setThreadLayers(n);r.inputs.push(...a.inputs),s.push(...a.sourceOutputs),r.outputs.push(...a.outputs),r=this.getWalletLayers(t,r,s,c,u);let i=x(r);if(!i.success)throw new Error("generate transaction failed!, errors: "+JSON.stringify(i.errors,null,"  "));const O=$(i.transaction,h),p=r.outputs.length-1;if(r.outputs[p].valueSatoshis=r.outputs[p].valueSatoshis-O,i=x(r),!i.success)throw new Error("generate transaction failed!, errors: "+JSON.stringify(i.errors,null,"  "));const b=i.transaction,S=z(b,s,{maximumTokenCommitmentLength:40});if(S!==!0&&h>0)throw S;let T=this.vm.verify({sourceOutputs:s,transaction:b});if(typeof T=="string")throw Error(T);let w=N(s)-N(b.outputs);w>O+10n&&(T=`Excessive fees: ${w} for ${O} byte tx`),U(s,t)==0n&&(T="Error checking token input");let E=U(s,t)-U(b.outputs,t);return E!==0n&&(T=`Swapping should not create destroy tokens, token difference: ${E}`),{sourceOutputs:s,transaction:b,verify:T}}}var Qt=R("<img/>"),$t=R('<img alt="Disconnected"/>'),zt=R('<section><div class="status svelte-13s96n2"><!> <!></div> <!></section>');function de(I,e){rt(e,!0);const t=lt.url.hostname=="vox.cash";t?C(pt):C(dt);const n=t?"bch.imaginary.cash":"chipnet.bch.ninja";let c=_(0),u=_(""),d=_(""),h=_(0),l=new Set,f=0,r,m=_(q([])),s=_(q([])),a=_(""),i=_(void 0),O="",p=_("");const b=async function(o){if(o.method==="blockchain.headers.subscribe"){let k=o.params[0];y(c,k.height,!0)}else o.method==="blockchain.scripthash.subscribe"?o.params[1]!==g(d)&&(y(d,o.params[1],!0),y(u,Nt[g(i).status],!0),S()):console.log(o)},S=()=>{clearTimeout(f),f=setTimeout(()=>{T(),w()},1500)},T=async function(){let o=await g(i).request("blockchain.scripthash.listunspent",g(p),"include_tokens");if(o instanceof Error)throw o;y(s,o,!0),y(h,vt(g(s),!0),!0),y(s,g(s).filter(k=>!k.token_data).filter(k=>k.height>0),!0)},w=async function(){let o=await g(i).request("blockchain.scripthash.listunspent",O,"include_tokens");if(o instanceof Error)throw o;let k=new Set(o.map(v=>`${v.tx_hash}":"${v.tx_pos}`));(g(m).length==0||l.intersection(k).size==0)&&y(m,o,!0),y(m,g(m).filter(v=>v.height>0),!0),g(m).sort((v,at)=>v.height-at.height)};st(async()=>{wt.StorageProvider=St,r=t?await xt.named("vox"):await Bt.named("vox"),y(a,Et(r.mnemonic,r.derivationPath.slice(0,-2),r.isTestnet),!0);let o=J(r.getDepositAddress());if(typeof o=="string")throw o;y(p,j(o.bytecode),!0);let k={installment:100n,period:0,recipient:o.bytecode,auth:Uint8Array.from([])};B.getAddress(k),y(i,new G(B.USER_AGENT,"1.4.1",n),!0),await g(i).connect(),g(i).on("notification",b),await g(i).subscribe("blockchain.scripthash.subscribe",O),await g(i).subscribe("blockchain.scripthash.subscribe",g(p)),await g(i).subscribe("blockchain.headers.subscribe")}),it(async()=>{await new G(B.USER_AGENT,"1.4.1",n).disconnect()});var E=zt(),D=V(E),F=V(D);ht(F,{get template(){return B.template}});var Z=X(F,2);{var tt=o=>{var k=Qt();W(()=>{L(k,"src",mt),L(k,"alt",g(u))}),A(o,k)},et=o=>{var k=$t();W(()=>L(k,"src",gt)),A(o,k)};ut(Z,o=>{g(u)=="CONNECTED"?o(tt):o(et,-1)})}M(D);var nt=X(D,2);Dt(nt),M(E),A(I,E),ct()}export{de as component,pe as universal};
//# sourceMappingURL=16.B20QITH1.js.map
