...
`,unlocks:"lock"},wallet_unlock:{name:"Wallet Unlock",script:"<key.schnorr_signature.all_outputs> <key.public_key>",unlocks:"wallet_lock"},lock:{lockingType:"p2sh32",name:"Photon Covenant",script:`  // pragma cashscript >= 0.10.0;
  ...
  //}`},wallet_lock:{lockingType:"standard",name:"Wallet Lock",script:"OP_DUP OP_HASH160 <$(<key.public_key> OP_HASH160)>  OP_EQUALVERIFY OP_CHECKSIG"},message_packet:{name:"Nonce & Target",script:"0xabcdef"}},Jt=["BCH_2026_05"],Zt={$schema:Yt,description:jt,name:Wt,entities:$t,scenarios:Qt,scripts:zt,supported:Jt},ea="@unspent/photon",ta={name:ea},ee=m("29972959d6f0dc766cdcb81bfaf8171c5605a64dd0a81fa46080f84ac87c9bef"),aa=m("8bc4c5974bb98a01d08ef9ba1b7208212b66678732519c4881d2fa924c2a159f");class E{static USER_AGENT=ta.name;static tokenAware=!0;static template=Zt;static compiler=bt(this.template);static vm=Rt();static getLockingBytecode(t={}){const i=this.compiler.generateBytecode({data:t,scriptId:"lock"});if(!i.success)throw new Error("Failed to generate bytecode, script: , "+JSON.stringify(i,null,"  "));return i.bytecode}static getScriptHash(t=!0){return Ce(this.getLockingBytecode(),t)}static getAddress(t="bitcoincash"){return vt(this.getLockingBytecode(),t,this.tokenAware)}static getSourceOutput(t){return{lockingBytecode:this.getLockingBytecode(),valueSatoshis:BigInt(t.value),token:t.token_data?{category:m(t.token_data.category),amount:BigInt(t.token_data.amount),nft:t.token_data.nft?{commitment:m(t.token_data.nft.commitment),capability:t.token_data.nft.capability}:void 0}:void 0}}static getInput(t,i,o){return{outpointIndex:t.tx_pos,outpointTransactionHash:m(t.tx_hash),sequenceNumber:i,unlockingBytecode:{data:{bytecode:{age:Ie(BigInt(i))},hdKeys:{addressIndex:0,hdPrivateKeys:{miner:o}}},compiler:this.compiler,script:"unlock",valueSatoshis:BigInt(t.value),token:t.token_data?{category:m(t.token_data.category),amount:BigInt(t.token_data.amount),nft:t.token_data.nft?{commitment:m(t.token_data.nft.commitment),capability:t.token_data.nft.capability}:void 0}:void 0}}}static getNextTarget(t,i){let o=t.height<=0?0:i-t.height,c=kt(m(t.token_data?.nft?.commitment).slice(4,36));return yt(c*(BigInt(o)+143n)/144n)}static getOutput(t,i,o,c,b){let l=t.height<=0?0:o-t.height;const A=this.getNextTarget(t,o);let I=Uint8Array.from([...It(b),...A]),w=Se.hash(I),P=Nt(c);if(typeof P=="string")throw P;let v=Et(P.node,0),u=Ut.signMessageHashSchnorr(v.privateKey,w);if(typeof u=="string")throw u;return{lockingBytecode:{data:{bytecode:{age:Ie(BigInt(l))},hdKeys:{addressIndex:0,hdPublicKeys:{miner:At(c).hdPublicKey}}},compiler:this.compiler,script:"lock"},valueSatoshis:BigInt(t.value-1500),token:{category:m(t.token_data.category),amount:BigInt(t.token_data?.amount)-BigInt(i),nft:{capability:"mutable",commitment:Uint8Array.from([...I,...u])}}}}static getRewardOutput(t,i,o=ee){let c=Be(i);if(typeof c=="string")throw c;return{lockingBytecode:c.bytecode,valueSatoshis:700n,token:{category:o,amount:BigInt(t)}}}static generateTemplate(t,i,o,c,b,l=0){const A=[],I=[];let w=b?m(b):ee,P=i.height<=0?0:t-i.height;const v=Math.floor(Number(BigInt(i.token_data.amount)/420000n))-1;let u={locktime:0,version:2,inputs:A,outputs:I};u.inputs.push(this.getInput(i,P,o)),u.outputs=[this.getOutput(i,v,t,o,l)],u.outputs.push(this.getRewardOutput(v,c,w));let B=wt(u);if(!B.success)throw new Error("generate transaction failed!, errors: "+JSON.stringify(B.errors,null,"  "));let x=B.transaction;const D=[this.getSourceOutput(i)];return this.vm.debug({inputIndex:0,sourceOutputs:D,transaction:x}),U(xt(x))}static getNextBatonUtxo(t){let i=Z(U(Ct(m(t)))),o=St(m(t));if(typeof o=="string")throw o;return{tx_pos:0,tx_hash:i,height:-1,value:Number(o.outputs[0].valueSatoshis),token_data:{nft:{commitment:U(o.outputs[0]?.token?.nft?.commitment),capability:"mutable"},amount:String(o.outputs[0]?.token?.amount),category:U(o.outputs[0]?.token?.category)}}}}var na=y('<h1>About Photons</h1> <p>Photon (PHOTON) is a solar punk meme token powering a decentralized energy oracle on Bitcoin Cash (BCH). It is a fairly distributed minable CashToken calculating its own transaction hash in BitcoinScript.</p> <p>Anyone can monetize their excess electricity by hashing for photons. Photons are released from the vault when someone finds the hash of the transaction is below a certain difficulty threshold. Each release of tokens must also update the hashing difficulty threshold. The difficulty <em>may</em> become correlated with the availability of excess free energy being absorbed by individuals globally.</p> <h2>Begin mining without sats</h2> <p>The Photon Vault keeps a cash balance to facilitate payments to miners. Each payout has an allowance of 1500 sats, for the dust accompanying a miner’s payout and the network fees for the transaction itself.</p> <h2>What is the PoW?</h2> <p>The PoW algorithm for Photons is Hash256(Secp256k1(Sha256)). Specifically, each transaction taking photons from the vault must return the mutable NFT baton with the following data updated:</p> <table><thead><tr><th align="left">Data</th><th align="left">Size</th></tr></thead><tbody><tr><td align="left">nonce</td><td align="left">4-bytes</td></tr><tr><td align="left">next target</td><td align="left">32-bytes (Little Endian)</td></tr><tr><td align="left">Schnorr signature of sha256(nonce + next target)</td><td align="left">64-bytes</td></tr></tbody></table> <p>When included in a transaction, satisfying a number of other requirements, if the double sha256 hash of the resulting transaction is less than the next target, the unlocking script for the photon vault can release a reward.</p> <h2>A decentralized energy oracle</h2> <p>Each miner taking tokens from the vault must update a dynamically changing difficulty value. The current <em>difficulty</em>, combined with the free market price of photons, creates a decentralized price oracle all miners contribute toward maintaining.</p> <p>If someone has already found a payout in this block, the difficulty gets one percent harder. If people stop taking tokens, the difficulty get easier each block.</p> <p>The vault contract also allows anyone to use the price baton for a fee (8000 sats). The price is intended to discourage resetting the oracle baton.</p> <h2>A simple Difficulty Adjustment Algorithm (DAA)</h2> <p>The target frequency for the price oracle is one update per block.</p> <p>The price oracle can be updated multiple times per block, but this makes mining 0.7% harder. If no reward is found in a block it gets 0.7% easier with each block. Mining becomes roughly 100% easier per day the oracle is not updated.</p> <p>The equation is a function of the baton transaction age (in block) and the previous difficulty target.</p> <p>NextTarget = ( PrevTarget * (143 + age) ) / 144</p> <p>The NextTarget MUST match the exact value given by the DAA, it may not be arbitrarily lowered by any miner.</p> <h2>Thanks</h2> <p>bitcoincashautist’s research provided valuable guidance on this idea & Adaptive Blocksize Limit .</p> <h2>See Also</h2> <ul><li>bitcoincashautist’s <a href="https://bitcoincashresearch.org/t/research-block-difficulty-as-a-price-oracle/1426" rel="nofollow">Research: Block difficulty as a price oracle</a></li> <li><a href="https://bitcoincashresearch.org/t/block-tops-btop-a-minable-cashtoken/" rel="nofollow">An early description</a> of this project (formerly BlockTops) Nov ‘25.</li> <li><a href="https://web.archive.org/web/20210128134553/https://mistcoin.org/" rel="nofollow">MIST: Mineable SLP Token - July 24, 2020</a></li> <li>SAFAs [forthcoming], which is a more sha256 ASIC friendly version of this contract.</li></ul>',1);function ia(M){var t=na();ht(44),T(M,t)}const oa="data:image/svg+xml,%3csvg%20width='400'%20height='400'%20version='1.1'%20viewBox='0%200%20106%20106'%20xmlns='http://www.w3.org/2000/svg'%20xmlns:xlink='http://www.w3.org/1999/xlink'%3e%3cdefs%3e%3cradialGradient%20id='a'%20cx='61'%20cy='5'%20r='53'%20gradientTransform='matrix(-.02%202%20-2%20-.02%2063%20-119)'%20gradientUnits='userSpaceOnUse'%3e%3cstop%20stop-color='%23753e0a'%20stop-opacity='.8'%20offset='0'/%3e%3cstop%20stop-color='%2316124d'%20offset='1'/%3e%3c/radialGradient%3e%3clinearGradient%20id='b'%20x1='53'%20x2='13'%20y1='61'%20y2='51'%20gradientUnits='userSpaceOnUse'%3e%3cstop%20stop-color='%23f6f4ca'%20offset='0'/%3e%3cstop%20stop-color='%23fff'%20stop-opacity='0'%20offset='1'/%3e%3c/linearGradient%3e%3cpattern%20id='bar'%20viewBox='0,0,10,10'%20width='10%25'%20height='10%25'%3e%3cpolygon%20style='opacity:0.2;fill:%23fff;'%20points='0,5%200,6%2010,6%2010,5'/%3e%3c/pattern%3e%3c/defs%3e%3cpath%20d='m10%203-8%208v86l8%208h86l8-8v-86l-8-8z'%20fill='url(%23a)'/%3e%3cpath%20d='m10%203-8%208v86l8%208h86l8-8v-86l-8-8z'%20fill='url(%23bar)'/%3e%3cpath%20d='m24%2022c26-0.8%2037%2072%2028%2072-10%200.2%200.1-59%2026-85'%20fill='none'%20stroke='url(%23b)'%20stroke-linecap='round'%20stroke-width='5'/%3e%3c/svg%3e",sa="data:image/svg+xml,%3csvg%20width='400'%20height='400'%20version='1.1'%20viewBox='0%200%20106%20106'%20xmlns='http://www.w3.org/2000/svg'%20xmlns:xlink='http://www.w3.org/1999/xlink'%3e%3cdefs%3e%3cradialGradient%20id='a'%20cx='61'%20cy='5'%20r='53'%20gradientTransform='matrix(-.02%202%20-2%20-.02%2063%20-119)'%20gradientUnits='userSpaceOnUse'%3e%3cstop%20stop-color='%230d0a75'%20stop-opacity='.8'%20offset='0'/%3e%3cstop%20stop-color='%2316124d'%20offset='1'/%3e%3c/radialGradient%3e%3clinearGradient%20id='b'%20x1='53'%20x2='13'%20y1='61'%20y2='51'%20gradientUnits='userSpaceOnUse'%3e%3cstop%20stop-color='%23f6f4ca'%20offset='0'/%3e%3cstop%20stop-color='%23fff'%20stop-opacity='0'%20offset='1'/%3e%3c/linearGradient%3e%3cpattern%20id='bar'%20viewBox='0,0,10,10'%20width='10%25'%20height='10%25'%3e%3crect%20style='opacity:0.2;fill:%23ffffff;'%20height='1'%20width='10'/%3e%3c/pattern%3e%3c/defs%3e%3cpath%20d='m10%203-8%208v86l8%208h86l8-8v-86l-8-8z'%20fill='url(%23a)'/%3e%3cpath%20d='m10%203-8%208v86l8%208h86l8-8v-86l-8-8z'%20fill='url(%23bar)'/%3e%3cpath%20d='m24%2022c26-0.8%2037%2072%2028%2072-10%200.2%200.1-59%2026-85'%20fill='none'%20stroke='url(%23b)'%20stroke-linecap='round'%20stroke-width='5'/%3e%3c/svg%3e";var ra=y('<meta name="description" content="Emit Photons Tokens"/>'),ca=y("<img/>"),la=y('<img alt="Disconnected"/>'),da=y('<button class="button svelte-1400jt5">go</button> <button class="button svelte-1400jt5">stop</button>',1),ua=y("<p> </p>"),fa=y('<h3>Vault Status</h3> <div class="swap svelte-1400jt5"><div class="svelte-1400jt5"><img width="50"/> <br/> <br/> <img width="18px"/></div></div> <h4>Current Difficulty</h4> <p>Current Target</p> <pre class="svelte-1400jt5"> </pre> <p>Previous Target</p> <pre class="svelte-1400jt5"> </pre> <br/> <br/> <br/> <p> </p> <pre class="svelte-1400jt5"> </pre>',1),ha=y('<button class="button svelte-1400jt5"> </button>'),pa=y('<section><div class="status svelte-1400jt5"> <sub>■</sub> <!> <!></div> <h1>Capture Photons</h1> <div class="swap svelte-1400jt5"><div class="svelte-1400jt5"><img width="50"/> <br/> </div></div> <div class="mining svelte-1400jt5"><!> <!> <p> </p></div> <!> <!></section>');function Ra(M,t){pt(t,!0);let i,o,c=f("STATUS_IDLE"),b=f(0),l=f(void 0),A="",I=f(void 0),w=f(""),P,v,u=f(ke([])),B=f(ke([])),x=f(""),D=f(0),te=f(0n),De=f(0),ae=f(0n),ne=f(0),L=f(""),_,X="";X=E.getScriptHash();const g=Tt.url.hostname=="vox.cash";let Re=g?"bch.imaginary.cash":"chipnet.bch.ninja";const ie=g?sa:oa,H=g?U(ee):U(aa),V=g?"BCH":"tBCH",C=g?"PHOTON":"tPHOTON",Me=g?"bitcoincash":"bchtest",Le=g?qt:Gt,He=function(e){if(e.method==="blockchain.headers.subscribe"){let a=e.params[0];r(b,a.height,!0)}else e.method==="blockchain.scripthash.subscribe"?e.params[1]!==A&&(A=e.params[1],r(L,Ae[_.status],!0),re(),se()):console.log(e)},oe=async function(){r(c,"STATUS_HALTED"),i.terminate(),await Ue(100),await ce(),await Ue(100)},q=async function(){let e=E.generateTemplate(n(b),n(l),n(x),P.getTokenDepositAddress(),H);window.Worker?i.postMessage({task:"START",template:e,key:n(x)}):console.log("Start mining called before worker init.")},Ve=async function(e){let a=await _.request("blockchain.transaction.broadcast",e);if(a instanceof Error)throw r(L,Ae[_.status],!0),a},Ke=async function(){let e=["OP_RETURN","BCMR",Se.hash(Dt("vox.cash/.well-known/bitcoin-cash-metadata-registry.json")),"vox.cash/.well-known/bitcoin-cash-metadata-registry.json"];P.tokenGenesis({cashaddr:E.getAddress(Me),amount:BigInt(21e14),value:50000000n,nft:{capability:"mutable",commitment:"00000000fffffffffffffffffffffffffffffffffffffffffffffffffffff00000000000"},bcmr_data:e})},se=async function(){let e=await _.request("blockchain.scripthash.listunspent",n(w),"include_tokens");if(e instanceof Error)throw e;r(u,e,!0),r(De,Ne(n(u),!0),!0),r(te,Ee(n(u),H),!0),r(u,n(u).filter(a=>!a.token_data).filter(a=>a.height>0),!0)},re=async function(){let e=await _.request("blockchain.scripthash.listunspent",X,"include_tokens");if(e instanceof Error)throw e;e=e.filter(a=>a.token_data?.category==H),e.length==1&&(r(l,e[0],!0),r(I,Z(n(l).token_data?.nft?.commitment.slice(8,72)),!0),n(c)=="STATUS_MINING"&&(await oe(),await q())),r(B,e,!0),r(ne,Ne(e,!0),!0),r(ae,Ee(e,H),!0)};async function ce(){if(window.Worker){r(c,"STATUS_IDLE"),o=void 0;const e=await lt(()=>import("../chunks/DvhAp_Sl.js"),[],import.meta.url);i=new e.default,i.onmessage=function(a){const{status:d,message:S}=a.data;switch(d&&r(c,d,!0),d){case"STATUS_BROADCAST":o=a.data.result,r(D,a.data.hashRate,!0);try{Ve(o)}catch(R){console.error(R)}r(l,E.getNextBatonUtxo(o),!0),q();break}}}else console.error("no worker")}ut(async()=>{Xt.StorageProvider=Kt,P=g?await we.named("vox"):await xe.named("vox"),v=g?await we.named("miner"):await xe.named("miner"),r(x,Bt(v.mnemonic,v.derivationPath.slice(0,-2),v.isTestnet),!0);let e=Be(P.getDepositAddress());if(typeof e=="string")throw e;r(w,Ce(e.bytecode),!0),_=new Mt(E.USER_AGENT,"1.4.1",Re),await _.connect(),_.on("notification",He),await _.subscribe("blockchain.scripthash.subscribe",X),await _.subscribe("blockchain.headers.subscribe"),re(),se(),ce()}),ft(async()=>{i.terminate(),await _.disconnect()});var G=pa();Pt("1400jt5",e=>{var a=ra();mt(()=>{_t.title="γ Photons"}),T(e,a)});var F=h(G),le=h(F,!0),de=s(le,3);Lt(de,{get template(){return E.template}});var Xe=s(de,2);{var qe=e=>{var a=ca();N(()=>{k(a,"src",Ht),k(a,"alt",n(L))}),T(e,a)},Ge=e=>{var a=la();N(()=>k(a,"src",Vt)),T(e,a)};K(Xe,e=>{n(L)=="CONNECTED"?e(qe):e(Ge,-1)})}p(F);var Y=s(F,4),ue=h(Y),j=h(ue),Fe=s(j,3);p(ue),p(Y);var W=s(Y,2),fe=h(W);{var Ye=e=>{var a=da(),d=ye(a),S=s(d,2);N(()=>{d.disabled=n(c)=="STATUS_MINING",S.disabled=n(c)=="STATUS_IDLE"}),J("click",d,()=>q()),J("click",S,()=>oe()),T(e,a)};K(fe,e=>{n(l)&&e(Ye)})}var he=s(fe,2);{var je=e=>{var a=ua(),d=h(a);p(a),N(()=>O(d,`${n(D)??""} Hash/s`)),T(e,a)};K(he,e=>{n(D)>0&&e(je)})}var pe=s(he,2),We=h(pe,!0);p(pe),p(W);var Oe=s(W,2);{var $e=e=>{var a=fa(),d=s(ye(a),2),S=h(d),R=h(S),me=s(R,3),_e=s(me,2),Pe=s(_e);p(S),p(d);var $=s(d,6),Ze=h($,!0);p($);var Q=s($,4),et=h(Q,!0);p(Q);var ge=s(Q),Te=s(ge,2),be=s(Te,2),z=s(be,3),tt=h(z);p(z);var ve=s(z,2),at=h(ve,!0);p(ve),N((nt,it,ot,st,rt,ct)=>{k(R,"src",ie),k(R,"alt",C),O(me,` ${nt??""}
				${C}`),O(_e,` ${it??""}
				${V} `),k(Pe,"src",Le),k(Pe,"alt",V),O(Ze,ot),O(et,st),O(ge,` Height: ${n(l).height??""} `),O(Te,` Next Payout: ${rt??""}
		${C}`),O(be,` Cash: ${ct??""} sats ${V}`),O(tt,`${C} category:`),O(at,n(l).token_data?.category)},[()=>(n(ae)/100000000n).toLocaleString(),()=>(n(ne)/1e8).toLocaleString(),()=>U(E.getNextTarget(n(l),n(b))),()=>Z(n(I)),()=>(Number(BigInt(n(l).token_data?.amount)/420000n)/1e8).toLocaleString(void 0,{maximumFractionDigits:5}),()=>n(l).value.toLocaleString()]),T(e,a)},Qe=e=>{var a=ha(),d=h(a);p(a),N(()=>O(d,`Mint Genesis Tx (0.5 ${V})`)),J("click",a,()=>Ke()),T(e,a)},ze=e=>{var a=dt("No tokens in vault");T(e,a)};K(Oe,e=>{n(l)&&n(l).value>0?e($e):g?e(ze,-1):e(Qe,1)})}var Je=s(Oe,2);ia(Je),p(G),N((e,a)=>{O(le,e),k(j,"src",ie),k(j,"alt",C),O(Fe,` ${a??""}
			${C}`),O(We,n(c))},[()=>n(b).toLocaleString(),()=>(n(te)/100000000n).toLocaleString(void 0,{maximumFractionDigits:5})]),T(M,G),Ot()}gt(["click"]);export{Ra as component,Da as universal};
//# sourceMappingURL=11.CgCujjoJ.js.map
