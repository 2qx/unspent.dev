...
// contract DripMine() {
    ...
    OP_DROP OP_1
// }
`}},re=["BCH_2023_05"],ce=0,le={$schema:ee,description:ne,name:se,entities:ae,scenarios:ie,scripts:oe,supported:re,version:ce},ue="@unspent/drip",pe={name:ue},ct=576,R=164,he=4392,de=1333036486;class T{static USER_AGENT=pe.name;static template=le;static compiler=qt(this.template);static getLockingBytecode(s={}){const e=this.compiler.generateBytecode({data:s,scriptId:"lock"});if(!e.success)throw new Error("Failed to generate bytecode, script: , "+rt(e));return e.bytecode}static getScriptHash(s=!0){return Ft(this.getLockingBytecode(),s)}static getAddress(s="bitcoincash"){return Ht(this.getLockingBytecode(),s)}static getOutput(s){let e=Math.round(s.value*he/de)-1;e=e<R?R:e;let o=s.value-e;return s.value>BigInt(ct+R)?{lockingBytecode:{compiler:this.compiler,script:"lock"},valueSatoshis:BigInt(o)}:{lockingBytecode:C("6a"),valueSatoshis:BigInt(0)}}static getInput(s){let e=s.value>BigInt(ct+R)?"unlock_return":"unlock_burn";return{outpointIndex:s.tx_pos,outpointTransactionHash:C(s.tx_hash),sequenceNumber:1,unlockingBytecode:{compiler:this.compiler,script:e,valueSatoshis:BigInt(s.value)}}}static processOutpoint(s){const e=[],o=[];o.push(this.getOutput(s)),e.push(this.getInput(s));const f=Yt({locktime:0,version:2,inputs:e,outputs:o});if(!f.success)throw new Error("generate transaction failed!, errors: "+rt(f.errors));return lt(zt(f.transaction))}}const me="data:image/svg+xml,%3csvg%20style='height:400px;width:400px'%20version='1.1'%20viewBox='0%200%2016%2016'%20xmlns='http://www.w3.org/2000/svg'%20xmlns:xlink='http://www.w3.org/1999/xlink'%3e%3cdefs%3e%3cradialGradient%20id='a'%20cx='7.24'%20cy='7.07'%20r='3.73'%20gradientTransform='matrix(-.551%201.72%20-1.8%20-.576%2022.2%20-.713)'%20gradientUnits='userSpaceOnUse'%3e%3cstop%20stop-color='%23b7ffff'%20stop-opacity='.839'%20offset='.05'/%3e%3cstop%20stop-color='%230052ef'%20offset='.95'/%3e%3c/radialGradient%3e%3c/defs%3e%3cpath%20d='m8.28%203.09c0.21-0.784%203.87%202.13%203.87%205.16%200%201.99-1.34%203.52-3.57%203.51-2.44%200-3.9-1.33-3.9-3.23%200-2.05%202.88-2.74%203.6-5.44z'%20fill='url(%23a)'/%3e%3c/svg%3e";var ve=u('<meta name="description" content="Release miner extractable value (MEV)!"/>'),_e=u("<img/>"),fe=u('<img alt="Disconnected"/>'),ge=u('<div class="row svelte-8n7es0"><button class="svelte-8n7es0"><img/></button></div>'),be=u("<p>No spendable outputs, check back in 10 minutes.</p>"),Oe=u('<div class="row svelte-8n7es0"><button disabled="" class="svelte-8n7es0"><img/></button></div>'),Ee=u("<p>No pending transactions</p>"),Pe=u('<div class="header svelte-8n7es0"><button class="svelte-8n7es0">Release all Miner Extractable Value (MEV)</button></div> <h3>Unspent Transaction Outputs (utxos)</h3> <div class="grid svelte-8n7es0"><!></div> <h3>Mempool Transactions</h3> <div class="grid svelte-8n7es0"><!></div>',1),ye=u('<div class="swap svelte-8n7es0"><p>Not connected?</p></div>'),xe=u('<section class="svelte-8n7es0"><div class="status svelte-8n7es0"><!> <!></div> <h1>Release miner extractable value (MEV)</h1> <!> <!> <qr-code><img width="50px" slot="icon" alt="drip MEV icon"/></qr-code> <pre id="deposit"> </pre></section>',2);function ze(N,s){Dt(s,!1);let e=Z([]),o,D="";D=T.getScriptHash();let f="",I=Z(""),V=new Set,H,Y=at.url.hostname=="vox.cash"?"bitcoincash":"bchtest",ut=at.url.hostname=="vox.cash"?"bch.imaginary.cash":"chipnet.bch.ninja";const pt=()=>{clearTimeout(H),H=setTimeout(()=>{V=new Set},1e4)},ht=function(t){t.method==="blockchain.scripthash.subscribe"?t.params[1]!==f&&(f=t.params[1],A(I,ot[o.status]),G()):console.log(t)},dt=async function(t){let n=await o.request("blockchain.transaction.broadcast",t);if(n instanceof Error)throw A(I,ot[o.status]),n},mt=function(){a(e).filter(t=>t.height>0).map((t,n)=>{z(t,n)})},z=async function(t,n){let h=T.processOutpoint(t),k=Gt(C(h));if(typeof k=="string")throw k;V.add(`${t.tx_hash}":"${t.tx_pos}`),pt();let g=Qt(lt(jt(C(h)))),w=Number(k.outputs[0].valueSatoshis);a(e).splice(n,1);let X=a(e).filter(b=>b.height>0||b.value<w?!0:b.value==w&&b.tx_hash<g).length;a(e).splice(X,0,{height:0,tx_hash:g,value:w}),A(e,a(e)),await dt(h)},G=async function(){let t=await o.request("blockchain.scripthash.listunspent",D,"exclude_tokens");if(t instanceof Error)throw t;let n=new Set(t.map(h=>`${h.tx_hash}":"${h.tx_pos}`));(a(e).length==0||V.intersection(n).size==0)&&A(e,t)};wt(async()=>{o=new Xt(T.USER_AGENT,"1.4.1",ut),await o.connect(),o.on("notification",ht),await o.subscribe("blockchain.scripthash.subscribe",D),G()}),Ut(async()=>{await o.disconnect()}),Lt();var S=xe();Ct("8n7es0",t=>{var n=ve();At(()=>{Mt.title="💧 Drip Mine"}),r(t,n)});var L=c(S),Q=c(L);Jt(Q,{get template(){return T.template}});var vt=_(Q,2);{var _t=t=>{var n=_e();y(()=>{m(n,"src",Kt),m(n,"alt",a(I))}),r(t,n)},ft=t=>{var n=fe();y(()=>m(n,"src",$t)),r(t,n)};M(vt,t=>{a(I)=="CONNECTED"?t(_t):t(ft,-1)})}l(L);var j=_(L,4);{var gt=t=>{var n=Pe(),h=F(n),k=c(h);l(h);var g=_(h,4),w=c(g);{var X=i=>{var d=W(),q=F(d);et(q,1,()=>a(e).filter(v=>v.height>0).slice(0,45),nt,(v,O,U)=>{var E=ge(),P=c(E),B=c(P);l(P),l(E),y(kt=>{m(B,"src",kt),m(B,"alt",a(O).tx_hash)},[()=>it(`0x${a(O).tx_hash}`,32)]),st("click",P,()=>z(a(O),U)),r(v,E)}),r(i,d)},b=tt(()=>a(e).filter(i=>i.height>0).length>0),Pt=i=>{var d=be();r(i,d)};M(w,i=>{a(b)?i(X):i(Pt,-1)})}l(g);var J=_(g,4),yt=c(J);{var xt=i=>{var d=W(),q=F(d);et(q,1,()=>a(e).filter(v=>v.height<=0),nt,(v,O)=>{var U=Oe(),E=c(U),P=c(E);l(E),l(U),y(B=>{m(P,"src",B),m(P,"alt",a(O).tx_hash)},[()=>it(`0x${a(O).tx_hash}`,32)]),r(v,U)}),r(i,d)},Tt=tt(()=>a(e).filter(i=>i.height<=0).length>0),It=i=>{var d=Ee();r(i,d)};M(yt,i=>{a(Tt)?i(xt):i(It,-1)})}l(J),st("click",k,()=>mt()),r(t,n)},bt=t=>{var n=ye();r(t,n)};M(j,t=>{a(I)=="CONNECTED"?t(gt):t(bt,-1)})}var K=_(j,2);te(K);var p=_(K,2);x(p,"id","qr1"),y(()=>x(p,"contents",T.getAddress(Y))),x(p,"module-color","#000"),x(p,"position-ring-color","#0052ef"),x(p,"position-center-color","#b7ffff"),x(p,"mask-x-to-y-ratio","1.2"),Vt(p,`width: 150px;
									height: 150px;
									margin: 0.5em auto;
									background-color: #fff;`);var Ot=c(p);l(p);var $=_(p,2),Et=c($,!0);l($),l(S),y(t=>{m(Ot,"src",me),Rt(Et,t)},[()=>T.getAddress(Y)]),r(N,S),Bt()}St(["click"]);export{ze as component,Ye as universal};
//# sourceMappingURL=8.BBD3v4jO.js.map
